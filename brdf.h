...
inline float rsqrt(float x) { return glm::inversesqrt(x); }
inline float saturate(float x) { return glm::clamp(x, 0.0f, 1.0f); }
inline glm::float2 mul(glm::float2x2 a, glm::float2 b) { return b * a; }
inline glm::float3 mul(glm::float3x3 a, glm::float3 b) { return b * a; }
...
{
	// Material properties
	glm::float3 specularF0;
	glm::float3 diffuseReflectance;
	...
	glm::float3 F; //< Fresnel term

	// Vectors
	glm::float3 V; //< Direction to viewer (or opposite direction of incident ray)
	glm::float3 N; //< Shading normal
	glm::float3 H; //< Half vector (microfacet normal)
	glm::float3 L; //< Direction to light (or direction of reflecting ray)
...
FUNCTION float beckmannAlphaToShininess(float alpha) {
	return 2.0f / glm::min(0.9999f, glm::max(0.0002f, (alpha * alpha))) - 2.0f;
}
...
FUNCTION float luminance(glm::float3 rgb)
{
	return dot(rgb, glm::float3(0.2126f, 0.7152f, 0.0722f));
}

FUNCTION glm::float3 baseColorToSpecularF0(const glm::float3 baseColor, const float metalness, const float reflectance = 0.5f) {
...
#endif
	return glm::lerp(glm::float3(minDielectricsF0, minDielectricsF0, minDielectricsF0), baseColor, metalness);
}

FUNCTION glm::float3 baseColorToDiffuseReflectance(glm::float3 baseColor, float metalness)
{
...
FUNCTION glm::float3 evalVoid(const BrdfData data) {
	return glm::float3(0.0f, 0.0f, 0.0f);
}

FUNCTION void evalIndirectVoid(const BrdfData data, glm::float2 u, OUT_PARAMETER(glm::float3) rayDirection, OUT_PARAMETER(glm::float3) weight) {
	rayDirection = glm::float3(0.0f, 0.0f, 1.0f);
	weight = glm::float3(0.0f, 0.0f, 0.0f);
}

FUNCTION glm::float3 sampleSpecularVoid(glm::float3 Vlocal, float alpha, float alphaSquared, glm::float3 specularF0, glm::float2 u, OUT_PARAMETER(glm::float3) weight) {
	weight = glm::float3(0.0f, 0.0f, 0.0f);
	return glm::float3(0.0f, 0.0f, 0.0f);
}

FUNCTION glm::float3 sampleSpecularHalfVectorVoid(glm::float3 Vlocal, glm::float2 alpha2D, glm::float2 u) {
	return glm::float3(0.0f, 0.0f, 0.0f);
}
...
FUNCTION glm::float4 getRotationToZAxis(glm::float3 input) {

	// Handle special case when input is exact or near opposite of (0, 0, 1)
	if (input.z < -0.99999f) return glm::float4(1.0f, 0.0f, 0.0f, 0.0f);

	return normalize(glm::float4(input.y, -input.x, 0.0f, 1.0f + input.z));
}

// Calculates rotation quaternion from vector (0, 0, 1) to the input vector
// Input vector must be normalized!
FUNCTION glm::float4 getRotationFromZAxis(glm::float3 input) {
...
FUNCTION glm::float4 invertRotation(glm::float4 q)
{
...
FUNCTION glm::float3 rotatePoint(glm::float4 q, glm::float3 v) {
	const glm::float3 qAxis = glm::float3(q.x, q.y, q.z);
...
FUNCTION glm::float3 sampleHemisphere(glm::float2 u, OUT_PARAMETER(float) pdf) {

	float a = sqrt(u.x);
	float b = TWO_PI * u.y;

	glm::float3 result = glm::float3(
		a * cos(b),
		a * sin(b),
		sqrt(1.0f - u.x));
...
FUNCTION glm::float3 sampleHemisphere(glm::float2 u) {
	float pdf;
...
FUNCTION glm::float3 evalFresnelSchlick(glm::float3 f0, float f90, float NdotS)
{
...
FUNCTION glm::float3 evalFresnelSchlickSphericalGaussian(glm::float3 f0, float f90, float NdotV)
{
...
FUNCTION glm::float3 evalFresnelHoffman(glm::float3 f0, float f82, float f90, float NdotS)
{
	const float alpha = 6.0f; //< Fixed to 6 in order to put peak angle for Lazanyi's error term at 82 degrees (f82)
	glm::float3 a = 17.6513846f * (f0 - f82) + 8.166666f * (glm::float3(1.0f, 1.0f, 1.0f) - f0);
	return saturate(f0 + (f90 - f0) * pow(1.0f - NdotS, 5.0f) - a * NdotS * pow(1.0f - NdotS, alpha));
}

FUNCTION glm::float3 evalFresnel(glm::float3 f0, float f90, float NdotS)
{
...
FUNCTION float shadowedF90(glm::float3 F0) {
	...
	return glm::min(1.0f, t * luminance(F0));
}
...
FUNCTION glm::float3 evalLambertian(const BrdfData data) {
...
FUNCTION glm::float3 evalPhong(const BrdfData data) {

	// First convert roughness to shininess (Phong exponent)
	float shininess = beckmannAlphaToShininess(data.alpha);

	glm::float3 R = reflect(-data.L, data.N);
	return data.specularF0 * (phongNormalizationTerm(shininess) * pow(glm::max(0.0f, dot(R, data.V)), shininess) * data.NdotL);
}

// Samples a Phong distribution lobe oriented along +Z axis
// Source: "Sampling Transformations Zoo" in Ray Tracing Gems by Shirley et al.
FUNCTION glm::float3 samplePhong(glm::float3 Vlocal, float shininess, glm::float2 u, OUT_PARAMETER(float) pdf) {
...
FUNCTION glm::float3 samplePhong(glm::float3 Vlocal, glm::float2 alpha2D, glm::float2 u) {
	float shininess = beckmannAlphaToShininess(dot(alpha2D, glm::float2(0.5f, 0.5f)));
	float pdf;
	return samplePhong(Vlocal, shininess, u, pdf);
}

// Sampling the specular BRDF based on Phong, includes normalization term
FUNCTION glm::float3 sampleSpecularPhong(glm::float3 Vlocal, float alpha, float alphaSquared, glm::float3 specularF0, glm::float2 u, OUT_PARAMETER(glm::float3) weight) {

	// First convert roughness to shininess (Phong exponent)
	float shininess = beckmannAlphaToShininess(alpha);

	float pdf;
	glm::float3 LPhong = samplePhong(Vlocal, shininess, u, pdf);
	...
	glm::float3 Nlocal = glm::float3(0.0f, 0.0f, 1.0f);
	glm::float3 lobeDirection = reflect(-Vlocal, Nlocal);
	glm::float3 Llocal = rotatePoint(getRotationFromZAxis(lobeDirection), LPhong);

	// Calculate the weight of the sample 
	glm::float3 Rlocal = reflect(-Llocal, Nlocal);
	float NdotL = glm::max(0.00001f, dot(Nlocal, Llocal));
	weight = glm::max(glm::float3(0.0f, 0.0f, 0.0f), specularF0 * NdotL);

	// Unoptimized formula was:
	//weight = specularF0 * (phongNormalizationTerm(shininess) * pow(glm::max(0.0f, dot(Rlocal, Vlocal)), shininess) * NdotL) / pdf;
...
FUNCTION float orenNayar(BrdfData data) {
	...
	float alpha = glm::max(thetaV, thetaL);
	float beta = glm::min(thetaV, thetaL);

	// Calculate cosine of azimuth angles difference - by projecting L and V onto plane defined by N. Assume L, V, N are normalized.
	glm::float3 l = data.L - data.NdotL * data.N;
	glm::float3 v = data.V - data.NdotV * data.N;
	...
	return (A + B * glm::max(0.0f, cosPhiDifference) * sin(alpha) * tan(beta));
}

FUNCTION glm::float3 evalOrenNayar(const BrdfData data) {
...
FUNCTION glm::float3 evalDisneyDiffuse(const BrdfData data) {
...
FUNCTION float frostbiteDisneyDiffuse(const BrdfData data) {
	...
	float energyFactor = glm::lerp(1.0f, 1.0f / 1.51f, data.roughness);
...
FUNCTION glm::float3 evalFrostbiteDisneyDiffuse(const BrdfData data) {
...
FUNCTION float Smith_G_a(float alpha, float NdotS) {
	return NdotS / (glm::max(0.00001f, alpha) * sqrt(1.0f - glm::min(0.99999f, NdotS * NdotS)));
}
...
FUNCTION float Smith_G2_Height_Correlated_GGX_Hammon(float alpha, float NdotL, float NdotV) {
	return 0.5f / (glm::lerp(2.0f * NdotL * NdotV, NdotL + NdotV, alpha));
}
...
// PDF is 'G1(NdotV) * D'
FUNCTION glm::float3 sampleGGXVNDF(glm::float3 Ve, glm::float2 alpha2D, glm::float2 u) {

	// Section 3.2: transforming the view direction to the hemisphere configuration
	glm::float3 Vh = normalize(glm::float3(alpha2D.x * Ve.x, alpha2D.y * Ve.y, Ve.z));
	...
	glm::float3 Nh = glm::float3(x, y, z) + Vh;
	...
	glm::float3 T1 = lensq > 0.0f ? glm::float3(-Vh.y, Vh.x, 0.0f) * rsqrt(lensq) : glm::float3(1.0f, 0.0f, 0.0f);
	glm::float3 T2 = cross(Vh, T1);
	...
	t2 = glm::lerp(sqrt(1.0f - t1 * t1), t2, s);

	// Section 4.3: reprojection onto hemisphere
	glm::float3 Nh = t1 * T1 + t2 * T2 + sqrt(glm::max(0.0f, 1.0f - t1 * t1 - t2 * t2)) * Vh;

#endif

	// Section 3.4: transforming the normal back to the ellipsoid configuration
	return normalize(glm::float3(alpha2D.x * Nh.x, alpha2D.y * Nh.y, glm::max(0.0f, Nh.z)));
}
...
FUNCTION float sampleGGXVNDFReflectionPdf(float alpha, float alphaSquared, float NdotH, float NdotV, float LdotH) {
	NdotH = glm::max(0.00001f, NdotH);
	NdotV = glm::max(0.00001f, NdotV);
	return (GGX_D(glm::max(0.00001f, alphaSquared), NdotH) * Smith_G1_GGX(alpha, NdotV, alphaSquared, NdotV * NdotV)) / (4.0f * NdotV);
}

// "Walter's trick" is an adjustment of alpha value for Walter's sampling to reduce glm::maximal weight of sample to about 4
...
FUNCTION float sampleWalterReflectionPdf(float alpha, float alphaSquared, float NdotH, float NdotV, float LdotH) {
	NdotH = glm::max(0.00001f, NdotH);
	LdotH = glm::max(0.00001f, LdotH);
	return Microfacet_D(glm::max(0.00001f, alphaSquared), NdotH) * NdotH / (4.0f * LdotH);
}

// Samples a microfacet normal for the Beckmann distribution using walter's method.
// Source: "Microfacet Models for Refraction through Rough Surfaces" by Walter et al.
// PDF is 'D * NdotH'
FUNCTION glm::float3 sampleBeckmannWalter(glm::float3 Vlocal, glm::float2 alpha2D, glm::float2 u) {
	float alpha = dot(alpha2D, glm::float2(0.5f, 0.5f));
	...
	return normalize(glm::float3(sinTheta * cos(phi), sinTheta * sin(phi), cosTheta));
}

// Samples a microfacet normal for the GG-X distribution using walter's method.
// Source: "Microfacet Models for Refraction through Rough Surfaces" by Walter et al.
// PDF is 'D * NdotH'
FUNCTION glm::float3 sampleGGXWalter(glm::float3 Vlocal, glm::float2 alpha2D, glm::float2 u) {
	float alpha = dot(alpha2D, glm::float2(0.5f, 0.5f));
...
FUNCTION glm::float3 sampleSpecularMicrofacet(glm::float3 Vlocal, float alpha, float alphaSquared, glm::float3 specularF0, glm::float2 u, OUT_PARAMETER(glm::float3) weight) {
	...
	float HdotL = glm::max(0.00001f, glm::min(1.0f, dot(Hlocal, Llocal)));
	const glm::float3 Nlocal = glm::float3(0.0f, 0.0f, 1.0f);
	float NdotL = glm::max(0.00001f, glm::min(1.0f, dot(Nlocal, Llocal)));
	float NdotV = glm::max(0.00001f, glm::min(1.0f, dot(Nlocal, Vlocal)));
	float NdotH = glm::max(0.00001f, glm::min(1.0f, dot(Nlocal, Hlocal)));
	glm::float3 F = evalFresnel(specularF0, shadowedF90(specularF0), HdotL);
...
FUNCTION glm::float3 evalMicrofacet(const BrdfData data) {

	float D = Microfacet_D(glm::max(0.00001f, data.alphaSquared), data.NdotH);
	float G2 = Smith_G2(data.alpha, data.alphaSquared, data.NdotL, data.NdotV);
	//glm::float3 F = evalFresnel(data.specularF0, shadowedF90(data.specularF0), data.VdotH); //< Unused, F is precomputed already
...
FUNCTION BrdfData prepareBRDFData(glm::float3 N, glm::float3 L, glm::float3 V, MaterialProperties material) {
	BrdfData data;
	...
	data.NdotL = glm::min(glm::max(0.00001f, NdotL), 1.0f);
	data.NdotV = glm::min(glm::max(0.00001f, NdotV), 1.0f);
...
FUNCTION glm::float3 evalCombinedBRDF(glm::float3 N, glm::float3 L, glm::float3 V, MaterialProperties material) {
	...
	if (data.Vbackfacing || data.Lbackfacing) return glm::float3(0.0f, 0.0f, 0.0f);

	// Eval specular and diffuse BRDFs
	glm::float3 specular = evalSpecular(data);
	glm::float3 diffuse = evalDiffuse(data);
	...
	return (glm::float3(1.0f, 1.0f, 1.0f) - data.F) * diffuse + specular;
#else
...
FUNCTION bool evalIndirectCombinedBRDF(glm::float2 u, glm::float3 shadingNormal, glm::float3 geometryNormal, glm::float3 V, MaterialProperties material, const int brdfType, OUT_PARAMETER(glm::float3) rayDirection, OUT_PARAMETER(glm::float3) sampleWeight) {
	...
	glm::float4 qRotationToZ = getRotationToZAxis(shadingNormal);
	glm::float3 Vlocal = rotatePoint(qRotationToZ, V);
	const glm::float3 Nlocal = glm::float3(0.0f, 0.0f, 1.0f);

	glm::float3 rayDirectionLocal = glm::float3(0.0f, 0.0f, 0.0f);
	...
	if (brdfType == DIFFUSE_TYPE) {
		...
		glm::float3 Hspecular = sampleSpecularHalfVector(Vlocal, glm::float2(data.alpha, data.alpha), u);
		...
		if (VdotH > 0.00001f)
		{
			sampleWeight *= (glm::float3(1.0f, 1.0f, 1.0f) - evalFresnel(data.specularF0, shadowedF90(data.specularF0), glm::min(1.0f, VdotH)));
		} else {
			return false;
		}
#else
		...
		float VdotH = glm::max(0.00001f, glm::min(1.0f, dot(Vlocal, Hspecular)));
		sampleWeight *= (glm::float3(1.0f, 1.0f, 1.0f) - evalFresnel(data.specularF0, shadowedF90(data.specularF0), VdotH));
#endif
#endif

	} else if (brdfType == SPECULAR_TYPE) {
		const BrdfData data = prepareBRDFData(Nlocal, glm::float3(0.0f, 0.0f, 1.0f) /* unused L vector */, Vlocal, material);
...
FUNCTION glm::float3 specularGGXReflectanceApprox(glm::float3 specularF0, float alpha, float NdotV)
{
#if USE_HEIGHT_CORRELATED_G2
	const glm::float2x2 A = glm::float2x2(
		0.995367f, -1.38839f,
		-0.24751f, 1.97442f
	);

	const glm::float3x3 B = glm::float3x3(
		1.0f, 2.68132f, 52.366f,
		16.0932f, -3.98452f, 59.3013f,
		-5.18731f, 255.259f, 2544.07f
	);

	const glm::float2x2 C = glm::float2x2(
		-0.0564526f, 3.82901f,
		16.91f, -11.0303f
	);

	const glm::float3x3 D = glm::float3x3(
	...
	const float E = dot(mul(A, glm::float2(1.0f, NdotV)), glm::float2(1.0f, alpha));
	const float F = dot(mul(B, glm::float3(1.0f, NdotV, NdotV3)), glm::float3(1.0f, alpha, alpha3));

	const float G = dot(mul(C, glm::float2(1.0f, NdotV)), glm::float2(1.0f, alpha));
	const float H = dot(mul(D, glm::float3(1.0f, NdotV2, NdotV3)), glm::float3(1.0f, alpha, alpha3));

	// Turn the bias off for near-zero specular 
	const float biasModifier = saturate(dot(specularF0, glm::float3(0.333333f, 0.333333f, 0.333333f)) * 50.0f);

	const float bias = glm::max(0.0f, (E / F)) * biasModifier;
	const float scale = glm::max(0.0f, (G / H));

	return glm::float3(bias, bias, bias) + glm::float3(scale, scale, scale) * specularF0;
}
